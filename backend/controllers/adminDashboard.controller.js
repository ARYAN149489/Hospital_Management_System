...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const activeDoctors = await Doctor.countDocuments({ approvalStatus: 'approved' });
    ...
    const recentPendingDoctors = await Doctor.find({ approvalStatus: 'pending' })
    ...
    const transformedAppointments = recentAppointments.map(apt => {
      ...
      if (aptObj.doctor && aptObj.doctor.user) {
        aptObj.doctor.name = `Dr. ${aptObj.doctor.user.firstName} ${aptObj.doctor.user.lastName}`;
      }
      return aptObj;
    });
    ...
    completedWithFees.forEach(apt => {
      if (apt.doctor && apt.doctor.consultationFee) {
        monthlyRevenue += apt.doctor.consultationFee;
      }
    });
