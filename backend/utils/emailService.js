...
class EmailService {
  ...
  async sendAppointmentConfirmation(arg1, arg2, arg3) {
    let appointment, patient, doctor, email;
    
    // Check if called with (patientEmail, appointment)
    if (typeof arg1 === 'string' && typeof arg2 === 'object') {
      ...
      // Called with (appointment, patient, doctor)
      appointment = arg1;
      patient = arg2;
      doctor = arg3;
    ...
    if (!appointment) return { success: false, error: 'No appointment details' };
  ...
  /**
   * Compatibility function: send alias for sendEmail
   */
  async send(options) {
  ...
  /**
   * Compatibility function: sendPrescriptionEmail
   */
  async sendPrescriptionEmail(email, prescription) {
    const subject = `New Prescription Added - ${prescription.prescriptionId}`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #2563eb;">New Prescription Added</h2>
        <p>A new prescription has been added to your medical profile.</p>
        <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
          <p><strong>Prescription ID:</strong> ${prescription.prescriptionId}</p>
          <p><strong>Diagnosis:</strong> ${prescription.diagnosis}</p>
          <p><strong>Date:</strong> ${new Date(prescription.createdAt).toLocaleDateString()}</p>
        </div>
        <p>Please log in to your patient portal to view the full prescription and download the PDF.</p>
        <p>Best regards,<br>The MediCare Plus Team</p>
      </div>
    `;
  ...
  /**
   * Compatibility function: sendLeaveApproval
   */
  async sendLeaveApproval(email, leave) {
    return await this.sendLeaveApprovalEmail(leave, leave.doctor);
  }

  /**
   * Compatibility function: sendLeaveRejection
   */
  async sendLeaveRejection(email, leave, rejectionReason) {
    leave.rejectionReason = rejectionReason;
    return await this.sendLeaveRejectionEmail(leave, leave.doctor);
  }

  /**
   * Compatibility function: sendAppointmentCancellation
   */
  async sendAppointmentCancellation(email, appointment) {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Hi ${patientName},</p>
        <p>The appointment scheduled on ${appointmentDate} at ${appointment.appointmentTime} has been cancelled.</p>
        ${appointment.cancellationReason ? `<p><strong>Reason for cancellation:</strong> ${appointment.cancellationReason}</p>` : ''}
        <p>If you did not request this cancellation or have questions, please contact our support team.</p>
        <p>Best regards,<br>The MediCare Plus Team</p>
      </div>
    `;
