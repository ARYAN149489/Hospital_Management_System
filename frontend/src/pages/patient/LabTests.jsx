  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <button type="button" onClick={() => navigate('?section=book-lab-test')} className="btn btn-primary btn-sm" style={{ border: 'none', cursor: 'pointer' }}>
          Book New Test
        </button>
      </div>
      ...
      {tests.length === 0 ? (
        <div className="glass-card" style={{ textAlign: 'center', padding: '60px' }}>
          ...
          <button type="button" onClick={() => navigate('?section=book-lab-test')} className="btn btn-primary btn-sm" style={{ border: 'none', cursor: 'pointer' }}>Book a Lab Test</button>
        </div>
      ) : (
            ...
            return (
              <div key={test._id} className="glass-card-sm" style={{ padding: '20px' }}>
                <div 
                  ...
                  style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', cursor: hasResults ? 'pointer' : 'default', outline: 'none' }} 
                  onClick={() => hasResults && setExpanded(isOpen ? null : test._id)}
                  onKeyDown={(e) => {
                    if (hasResults && (e.key === 'Enter' || e.key === ' ')) {
                      e.preventDefault();
                      setExpanded(isOpen ? null : test._id);
                    }
                  }}
                >
                ...
                {hasResults && isOpen && (
                  <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--surface-high)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); handlePrint(test); }} className="btn btn-secondary btn-sm" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
